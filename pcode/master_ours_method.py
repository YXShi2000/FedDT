...
class Master(object):
    def __init__(self, conf):
        ...
        if conf.partitioned_by_user:
            ...
            # convert to hog feature. only single test dataset is considered temporarily.
            # type of self.dataset["test"]: "torchvision.datasets.cifar.CIFAR10".
            if conf.use_hog_feature:
                tmp_dataset_data = []
                print("master test data converting to hog features...")
                for idx, image in enumerate(self.dataset["test"].data):
                    image = 0.299 * image[:, :, 0] + 0.587 * image[:, :, 1] + 0.114 * image[:, :, 2]
                    hog_feature = hog(image, orientations=8, pixels_per_cell=(8, 8),
                                      cells_per_block=(2, 2))
                    tmp_dataset_data.append(hog_feature)
                tmp_self_dataset = deepcopy(self.dataset["test"])  # must use deepcopy!
                tmp_self_dataset.data = np.array(tmp_dataset_data)
                hog_test_loader, _ = create_dataset.define_data_loader(
                    conf, tmp_self_dataset, is_train=False
                )
            else:
                hog_test_loader = None
            # print("test_loader.data shape:", test_loader.dataset.data.shape, "||  test_loader.targets shape:",
            #       len(test_loader.dataset.targets))
            # print("hog_test_loader.data shape:", self.dataset["test"].data.shape, "||  hog_test_loader.targets shape:",
            #       len(self.dataset["test"].targets))

            self.test_loaders, self.hog_test_loader = [test_loader], [hog_test_loader]
        ...
        try:
            self.num_arch = int(self.conf.n_clients / conf.arch_info["num_clients_per_model"])
        except:
            pass

        # initialize variables for storing accuracy.
        self.logits_remain = 10
        self.arch_acc, self.arch_acc2, self.arch_acc_per_round, self.arch_acc_per_round2 = {}, {}, {}, {}
        self.avg_perfs_lst, self.avg_perfs_lst2 = [], []
        self.no_fedavg_perf_lst, self.no_fedavg_perf_best_lst = [], []
        self.local_perf_lst = []
        self.local_model_avg_perfs_lst = []
        self.logits_remain_lst = []
    ...
    def run(self):
        for comm_round in range(1, 1 + self.conf.n_comm_rounds):
            start_time_per_round = time.time()
            ...
            print(self.arch_acc_per_round2)
            print(self.master_performance_lst)

            # evaluate the aggregated model.
            self.conf.logger.log(f"Master finished one round of federated learning.\n")
            self.time_lst.append(time.time() - start_time_per_round)
    ...
    def _activate_selected_clients(
        ...
        if self.conf.dynamic_local_n_epochs:
            tmp_acc_lst = []
            for key in self.arch_acc.keys():
                tmp_acc_lst.append(self.arch_acc[key])
            activation_msg = torch.zeros((6, len(selected_client_ids)))
        else:
        ...
        if self.conf.dynamic_local_n_epochs:
            activation_msg[3, :] = tmp_acc_lst[0]
            activation_msg[4, :] = tmp_acc_lst[1]
            activation_msg[5, :] = tmp_acc_lst[2]
    ...
    def _aggregate_model_and_evaluate(self, flatten_local_models):
        ...
        if self.aggregator.aggregate_fn is not None:
            ...
            # heterogenous clients.
                ...
                performance = None
                # evaluate fedavg_models on testset
                for _arch, _fedavg_model in fedavg_models.items():
                    ...
                    tmp_performance = master_utils.get_avg_perf_on_dataloaders(
                        ...
                        self.criterion,
                        self.metrics,
                        self.hog_test_loader if (
                                _arch in self.ml_model_lst and self.conf.use_hog_feature) else self.test_loaders,
            ...
            # client_models, self.master_model, before_clean_var, after_clean_var, perfs, local_model_avg_perfs = self.aggregator.aggregate(
            # self.logits_remain = 8
            self.logits_remain_lst.append(self.logits_remain)
            client_models, self.master_model, before_clean_var, after_clean_var = self.aggregator.aggregate(
                master_model=self.master_model,
                client_models=self.client_models,
                fedavg_model=fedavg_model,
                fedavg_models=fedavg_models,
                fedavg_models_lst=None,
                flatten_local_models=flatten_local_models,
                performance=performance,
                hog_test_loader=self.hog_test_loader[0],
                logits_remain = self.logits_remain,
            )
            if len(self.after_clean_var_lst) == 0 or after_clean_var < self.after_clean_var_lst[-1]:
                if self.logits_remain > 2:
                    self.logits_remain -= 1
            else:
                if self.logits_remain < 10:
                    self.logits_remain += 1
            self.before_clean_var_lst.append(before_clean_var)
            self.after_clean_var_lst.append(after_clean_var)
            # self.no_fedavg_perf_lst.append(perfs)
            # self.no_fedavg_perf_best_lst.append(max(perfs))
            # self.local_model_avg_perfs_lst.append(round(local_model_avg_perfs,3))
            ...
            if same_arch:
                ...
                for _arch, _fedavg_model in fedavg_models.items():
                    ...
                    tmp_performance = master_utils.get_avg_perf_on_dataloaders(
                        ...
                        self.criterion,
                        self.metrics,
                        data_loaders=self.hog_test_loader if (
                                _arch in self.ml_model_lst and self.conf.use_hog_feature) else self.test_loaders,
                        label=f"fedag_test_loader_{_arch}",
                    )
                    tmp_top1 = str2top1(str(tmp_performance))
        ...
        master_performance = master_utils.do_validation(
            self.conf,
            self.coordinator,
            self.master_model,
            self.criterion,
        ...
        # if self.master_performance_lst == [] or str2top1(str(master_performance)) > max(self.master_performance_lst):
        #     torch.save(self.master_model, "global_resnet2.pth")
        self.master_performance_lst.append(str2top1(str(master_performance)))
        # exclude fedavg, because fedavg does not involve knowledge distillation
        if self.aggregator.aggregate_fn is not None:
    ...
    def _print_result(self):
        try:
            ...
            for (k1, v1), (k2, v2) in zip(self.arch_acc_per_round.items(), self.arch_acc_per_round2.items()):
                ...
                tmp_dict[k1] = tmp_lst
            print(tmp_dict)
            print("before_clean_var_lst:", self.before_clean_var_lst)
            print("after_clean_var_lst:", self.after_clean_var_lst)
            print("self.no_fedavg_perf_lst", self.no_fedavg_perf_lst)
            print("self.no_fedavg_perf_best_lst", self.no_fedavg_perf_best_lst)
            # print("self.local_perf_lst", self.local_perf_lst)
            print("self.local_model_avg_perfs_lst", self.local_model_avg_perfs_lst)
            print("self.logits_remain_lst", self.logits_remain_lst)
            print(self.time_lst)
            return
        except:
            pass
