...
import pcode.aggregation.ours_method as ours_method
...
class Aggregator(object):
    ...
    def _define_aggregate_fn(self):
        ...
        elif self.conf.fl_aggregate["scheme"] == "ours_method":
            self.aggregate_fn = self._s11_ours_method()
        else:
    ...
    def _s11_ours_method(self):
        def f(**kwargs):
            _client_models = ours_method.aggregate(
                conf=self.conf,
                ...
                hog_distillation_sampler=None if not self.conf.use_hog_feature else self.hog_data_info["sampler"],
                distillation_data_loader=self.data_info["data_loader"],
                hog_distillation_data_loader=None if not self.conf.use_hog_feature
                else self.hog_data_info["data_loader"],
                val_data_loader=self.data_info["self_val_data_loader"],
                hog_val_data_loader=None if not self.conf.use_hog_feature
                else self.hog_data_info["self_val_data_loader"],
                test_data_loader=self.test_loaders[0],
                logits_remain=kwargs["logits_remain"],
            )
            return _client_models
