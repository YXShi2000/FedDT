...
def aggregate(
    ...
    #     fedavg_model_lst.append(fedavg_model)
    ...
    #         display=False,
    #     )
    #     tmp_perfs.append(perf["top1"])
    # local_model_avg_perfs = sum(tmp_perfs) / len(tmp_perfs)
    ...
    kt = NoiseKTSolver(
        ...
        clean_logits=False
        if "logits_clean" not in fl_aggregate
        else fl_aggregate["logits_clean"],
    ...
    master_model = kt.server_student.cpu()
    ...
    for arch, fedavg_model in fedavg_models.items():
        ...
        #     continue
        # if "resnet" in arch:
        #     _client_models[arch] = master_model
        #     continue
        kt = NoiseKTSolver(
            conf=conf,
            arch=arch,
            teacher_models=[master_model],
    ...
    try:
        ...
        pass
    torch.cuda.empty_cache()
    # return _client_models, master_model, round(before_clean_var, 4), round(after_clean_var, 4), perfs, local_model_avg_perfs
    return _client_models, master_model, round(before_clean_var, 4), round(after_clean_var, 4),
...
class NoiseKTSolver(object):
    ...
    def distillation(self):
        ...
        client_weights = self._get_client_weights(self.base_solver)
        ...
        while n_pseudo_batches < self.total_n_server_pseudo_batches:
            ...
            for _ in range(self.server_local_steps):
                ...
                stud_avg_loss, (before_clean_var, after_clean_var) = self.update_student(
                    ...
                    # clean_logits=False if "resnet" in self.arch else self.clean_logits,
                    clean_logits=self.clean_logits,
                    batch_size=self.conf.batch_size,
                    logits_remain=self.logits_remain,
                )
                # print("-------------------")
                # print(before_clean_var)
                # print(after_clean_var)
                # print("-------------------")
                if after_clean_var > 0.00001:
    ...
    def _get_client_weights(self, base_solver):
    ...
    def update_student(
        ...
        if update_student_scheme == "avg_losses":
            ...
            # before_clean_var = np.sum(torch.var(F.softmax(_teacher_avg_logits, dim=1), dim=1).cpu().numpy()) / batch_size
            student_logits, _ = student_logits_activations[0]

            if clean_logits:
                # before_clean_var, after_clean_var = 1, 1
                ...
                # print("-------------------")
                # print(type(teacher_avg_logits))
                # print(teacher_avg_logits)
                # print(teacher_logits[0].shape[0], teacher_logits[0].shape[1])
                for i in range(teacher_logits[0].shape[0]):
                    for j in range(teacher_logits[0].shape[1]):
                        ...
                        # worst_logit, worst_logit2 = 0, 0
                        # for idx, logit in enumerate(tmp_logit_lst):
                        #     _abs = abs(mean_logit - logit)
                        #     if _abs > worst_logit:
                        #         if _abs > worst_logit2:
                        #             worst_logit = worst_logit2
                        #             worst_logit2 = _abs
                        #         else:
                        #             worst_logit = _abs
                        # teacher_avg_logits[i][j] = (sum(tmp_logit_lst) - worst_logit - worst_logit2) / (
                        #         len(tmp_logit_lst) - 2)
                # after_clean_var = np.sum(torch.var(F.softmax(teacher_avg_logits, dim=1), dim=1).cpu().numpy()) / 32
                after_clean_var = 0
